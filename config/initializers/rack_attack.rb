...
class Rack::Attack
  ...
  throttle('throttle_authenticated_api', limit: 2_500, period: 5.minutes) do |req|
  ...
  throttle('throttle_api_media', limit: 600, period: 30.minutes) do |req|
    req.authenticated_user_id if req.post? && req.path.match?(%r{\A/api/v\d+/media\z}i)
  end

  throttle('throttle_media_proxy', limit: 600, period: 10.minutes) do |req|
